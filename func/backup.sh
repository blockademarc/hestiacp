...
# Defining ftps storage function
ftps_backup() {
    # Checking config
    if [ ! -e "$HESTIA/conf/ftps.backup.conf" ]; then
        error="ftps.backup.conf doesn't exist"
        rm -rf "$tmpdir"
        rm -f "$BACKUP"/"$user".log
    ...
    if [ -z "$HOST" ] || [ -z "$USERNAME" ] || [ -z "$PASSWORD" ]; then
        error="Can't parse ftps backup configuration"
        rm -rf "$tmpdir"
        rm -f "$BACKUP"/"$user".log
        echo "$error" |$SENDMAIL -s "$subj" "$email" "$notify"
        sed -i "/ $user /d" "$HESTIA"/data/queue/backup.pipe
    ...
    # Debug info
    echo -e "$(date "+%F %T") Remote: ftps://$HOST$BPATH/$user.$backup_new_date.tar"

    # Checking ftps connection
    fconn=$(ftpsc)
    ferror=$(echo "$fconn" |grep -i -e failed -e error -e "Can't" -e "not conn")
    if [ ! -z "$ferror" ]; then
        error="Error: can't login to ftps ftps://$USERNAME@$HOST"
        rm -rf "$tmpdir"
        rm -f "$BACKUP"/"$user".log
        echo "$error" |$SENDMAIL -s "$subj" "$email" "$notify"
        sed -i "/ $user /d" "$HESTIA"/data/queue/backup.pipe
    ...
    if [ ! -z "$ftps_result" ] ; then
        ...
        echo "$error" |$SENDMAIL -s "$subj" "$email" "$notify"
        sed -i "/ $user /d" "$HESTIA"/data/queue/backup.pipe
        check_result "$E_FTPS" "$error"
    fi

    # Checking retention
    if [ -z "$BPATH" ]; then
        backup_list=$(ftpsc "ls" |awk '{print $9}' |grep "^$user\.")
    else
        backup_list=$(ftpsc "cd $BPATH" "ls" |awk '{print $9}' |grep "^$user\.")
    fi
    backups_count=$(echo "$backup_list" |wc -l)
    if [ "$backups_count" -ge "$BACKUPS" ]; then
        ...
        for backup in $(echo "$backup_list" |head -n $backups_rm_number); do 
            backup_date=$(echo "$backup" |sed -e "s/$user.//" -e "s/.tar$//")
            echo -e "$(date "+%F %T") Rotated ftps backup: $backup_date" |\
                tee -a "$BACKUP"/"$user".log
            if [ -z "$BPATH" ]; then
                ftpsc "rm $backup"
            else
                ftpsc "cd $BPATH" "rm $backup"
            fi
        done
    fi
    ...
    if [ "$localbackup" = 'yes' ]; then
        cd "$BACKUP"
        if [ -z "$BPATH" ]; then
            ftpsc "put $user.$backup_new_date.tar"
         else
            ftpsc "cd $BPATH" "put $user.$backup_new_date.tar"
        fi
    else
        cd "$tmpdir"
        tar -cf "$BACKUP"/"$user"."$backup_new_date".tar .
